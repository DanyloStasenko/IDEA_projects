/*
    ...
    many times in one text is quite tiresome. Let's consider a word too long, if its length is
    strictly more than 10 characters. All too long words should be replaced with a special abbreviation.
...
arr.forEach(function(item, i, arr) {
    ...
    if (item.length > 10){
        alert(item.charAt(0) + "" + (item.length - 2) + "" + item.charAt(item.length-1));
    } 
